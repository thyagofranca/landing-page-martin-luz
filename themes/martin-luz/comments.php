	...
	<div class="area-comments">
		<?php if ( have_comments() && comments_open() ) : ?>
	        ...
	        <?php $comments_arg = array(
	            ...
	            'comment_notes_before' => '<p>Seu e-mail não será divulgado.</p>',

		            'fields' => apply_filters( 'comment_form_default_fields', array(
		                'author' => '<div class="row"><div class="col-md-6 col-sm-12"><div class="form-group">' .
		                            '<label class="control-label" for="author">' . __('Digite seu nome') . '</label>' . 
		                            ( $req ? '<span>*</span>' : '' ) .
		                ...
		                'email' => '<div class="col-md-6 col-sm-12"><div class="form-group">' .
		                           '<label class="control-label" for ="email">' . __('Digite seu e-mail') . '</label>' .
		                           ( $req ? '<span>*</span>' : '' ) .
		                           '<input class="form-control" id="email" name="email" type="text" value="' .
		                           esc_attr( $commenter['comment_author_email'] ) . '" ' . $aria_req . '/></div></div></div>',
		            ) ),
		                'comment_field' => '<p>' . 
		                                   '<textarea id="comment" class="form-control" placeholder=" Digite sua  mensagem..." name="comment" rows="3"
